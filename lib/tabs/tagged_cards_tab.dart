...
class TaggedCardsTab extends StatefulWidget {
  ...
  final String tag;
  @override
  State<TaggedCardsTab> createState() => _TaggedCardsTabState();
}

class _TaggedCardsTabState extends State<TaggedCardsTab> {
  ...
  Future<List<MyCard>> getInitialCards() async {
    cards = await cardController.getInitialTaggedCards(limit, widget.tag);
    return cards;
  }

  Future<void> getNextCards(MyCard card) async {
    final newCards = await cardController.getNextTaggedCards(card, limit, widget.tag);
